...
void* threadConvolute(void* arg) {
    ...
    for (int row = args->startRow; row < args->endRow; row++) {
        for (int x = 0; x < args->srcImage->width; x++) {
...
//Returns: Nothing
void convolute(Image* srcImage,Image* destImage,Matrix algorithm, int nthreads){
    pthread_t threads[nthreads];
    ThreadArgs args[nthreads];
    int rowsPerThread = srcImage->height / nthreads;
    ...
    for (int i = 0; i < nthreads; i++) {
        ...
        args[i].endRow = (i == nthreads - 1) ? srcImage->height : (i + 1) * rowsPerThread;
